...
public class TCBService {
    public class func initialize(enviroment: Environment, auth: TCBServiceAuth) throws {
        guard let configFilePath = enviroment.configFilePath else {
            fatalError("Cound not load config file")
        }
...
//        let configFileURL = URL(fileURLWithPath: configFilePath)
        ...
        let userProfileDataProvider = UserProfileDataProvider(type: .remote(path: "/api"))
        try Backbase.register(client: AccessControlClient(dataProvider: accessControlDataProvider))
        try Backbase.register(client: UserProfileClient(dataProvider: userProfileDataProvider))
    }
...
private extension TCBService {
    class func buildAuthClient(type: TCBServiceAuth) -> AuthClient {
        ...
        switch type {
        case .community:
            return TCBCommunityAuthClient(configuration: Backbase.configuration())
        case .identity:
...
extension Domain.TCBResult {
    static func from(_ result: TCBResult<T>) -> Domain.TCBResult<T> {
        switch result {
        case .success(let value):
